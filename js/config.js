...
const INIZIO = {
  ...
  // --- PROXIMAS FECHAS (edita aqui cada semana) ---
  fechas: [
    { dia: 'Viernes',  fecha: '10 Abril',  hora: '20:00', plazas: 8 },
    { dia: 'Sábado',   fecha: '11 Abril',  hora: '19:30', plazas: 8 },
    { dia: 'Viernes',  fecha: '25 Abril',  hora: '20:00', plazas: 8 },
  ],
...
// Inyectar numero en todos los links de WhatsApp
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Renderizar proximas fechas si existe el contenedor
  const fechasContainer = document.getElementById('proximas-fechas');
  if (fechasContainer && INIZIO.fechas.length > 0) {
    fechasContainer.innerHTML = INIZIO.fechas.map(f => `
      <div class="fecha-card ${f.plazas <= 2 ? 'fecha-card--urgente' : ''}">
        <div class="fecha-card__dia">${f.dia}</div>
        <div class="fecha-card__fecha">${f.fecha}</div>
        <div class="fecha-card__hora">${f.hora}h</div>
        <div class="fecha-card__plazas">
          ${f.plazas <= 2
            ? `<span class="plazas--urgente">⚡ ${f.plazas} plazas</span>`
            : `<span class="plazas--ok">${f.plazas} plazas</span>`
          }
        </div>
        <a href="https://wa.me/${INIZIO.whatsapp}?text=Hola!%20Quiero%20reservar%20para%20el%20${encodeURIComponent(f.dia + ' ' + f.fecha)}%20a%20las%20${f.hora}h"
